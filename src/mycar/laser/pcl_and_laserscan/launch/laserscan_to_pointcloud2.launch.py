...
def generate_launch_description():
    ...
    #启动laserscan_to_pointcloud2节点
    laserscan_to_pointcloud2_node = Node(
        ...
        parameters=[
            {
                'target_frame': 'laser' #与激光雷达坐标系保持一致,不写也行
            }
        ],
        remappings=[
            ('/scan_in','/scan'),  #输入激光雷达数据话题
            ('/cloud','/sdy_pointcloud2') #输出点云数据话题
        ]
    )
    ld.add_action(laserscan_to_pointcloud2_node)
